...
> **Windows note.** Use `python -m pip`, not bare `pip` — if you have more than
> one Python installed, bare `pip` can install into a different one than
> `python` runs. Nothing here needs a C++ compiler: `requirements.txt`
> deliberately avoids `eth-tester[py-evm]`, whose `safe-pysha3` dependency has
> no Windows wheels and fails to build without Visual C++ Build Tools.
